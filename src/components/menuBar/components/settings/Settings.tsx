...
const Settings: FC<SettingsProps> = ({ open, setOpen }) => {

    const mobileView = useMediaQuery('(max-width:600px)');

    const [selectedSection, setSelectedSection] = useState<null | { componentName: string, label: string }>(null)
    ...
    const handleSelect = (componentName: string, label: string) => {
                ...
                <Stack >
                    ...
                    <Stack direction="row" sx={{ height: 400, position: "relative" }} >
                        ...
                        <Stack sx={{ flexGrow: 1, p: 2, display: selectedSection || !mobileView ? "block" : "none" }}>
                            <Stack direction="row" gap={1} sx={{ display: (mobileView && selectedSection) ? "flex" : "none", pb: 3, cursor: "pointer" }} onClick={() => setSelectedSection(null)}>
                                <ArrowBackIcon />
